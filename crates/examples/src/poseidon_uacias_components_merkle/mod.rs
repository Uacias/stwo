//! Merkle tree verification circuit with LogUp
//!
//! Architecture:
//! - MerkleComputingComponent: Computes Merkle path from leaf to root using Poseidon hash
//!   (yields computed root via LogUp)
...
// LogUp relation for 8-element Merkle roots (rate part only)
relation!(MerkleRelation, RATE);
...
pub fn gen_is_level_start_column(
    ...
    // Set even rows (0, 2, 4, ...) within active range to 1
    for row in (0..n_active_rows).step_by(2) {
...
impl MerkleStatement0 {
    ...
    pub fn log_sizes(&self) -> TreeVec<Vec<u32>> {
        // For Merkle (SPONGE): we need to calculate the actual column counts
        // Computing: message (8) + initial_state (16) +
        //            intermediates (full1: 4*16, partial: 14, full2: 4*16) + final_state (16)
        // = 8 + 16 + 64 + 14 + 64 + 16 = 182 columns
        const MERKLE_COMPUTING_N_COLUMNS: usize = RATE + N_STATE + (N_HALF_FULL_ROUNDS * N_STATE) + N_PARTIAL_ROUNDS + (N_HALF_FULL_ROUNDS * N_STATE) + N_STATE;

        // Scheduler: computed_root (8) + expected_root (8) = 16 columns
        const MERKLE_SCHEDULER_N_COLUMNS: usize = RATE + RATE;
...
/// Statement 1 for Merkle verification: Public inputs and LogUp sums
/// This is mixed into the channel after drawing PoseidonRelation
#[derive(Clone, Debug)]
pub struct MerkleStatement1 {
    pub expected_root: [BaseField; RATE], // Public input: the expected Merkle root
...
impl MerkleStatement1 {
    pub fn mix_into(&self, channel: &mut Blake2sChannel) {
        // Mix expected_root as public input
        for elem in &self.expected_root {
            channel.mix_felts(&[SecureField::from(*elem)]);
        }
        // Mix claimed sums
        channel.mix_felts(&[
...
> {
    ...
    // In sponge construction: each tree level = 2 rows, so need 2*depth rows
    let min_rows = (depth * 2).max(1);
    ...
    // Note: In sponge construction, we have 2*depth active rows (2 rows per tree level)
    ...
    let proof = prove(&[&component_computing, &component_scheduler], channel, commitment_scheme)?;
...
pub fn verify_merkle(
    ...
    let component_computing = MerkleComputingComponent::new(
        ...
        MerkleComputingEval {
            ...
            is_first_id: is_first_id.clone(),
            is_active_id: is_active_column_id(log_size, depth * 2),
            is_level_start_id: is_level_start_column_id(log_size, depth),
            is_last_id: is_last_column_id(log_size, depth),
        },
    ...
    stwo::core::verifier::verify(&[&component_computing, &component_scheduler], channel, commitment_scheme, proof)?;
...
mod tests {
    ...
    fn test_merkle_proof() {
        ...
        // Create leaf value
        let leaf: [BaseField; RATE] = std::array::from_fn(|i| BaseField::from_u32_unchecked((100 + i) as u32));

        // For a real Merkle tree, we'd compute actual hashes
        // For this test, we'll use dummy sibling values and compute the path
        // to get the expected root
        ...
        fn hash_two_nodes(left: [BaseField; RATE], right: [BaseField; RATE]) -> [BaseField; RATE] {
            use super::{apply_external_round_matrix, apply_internal_round_matrix, pow5, EXTERNAL_ROUND_CONSTS, INTERNAL_ROUND_CONSTS, N_HALF_FULL_ROUNDS, N_PARTIAL_ROUNDS};

            // SPONGE CONSTRUCTION: Two sequential absorptions

            // ===== FIRST ABSORPTION: left node =====
            let mut state: [BaseField; N_STATE] = [BaseField::from_u32_unchecked(0); N_STATE];
            state[0..RATE].copy_from_slice(&left);
            // Capacity is zeros for first absorption

            // Poseidon permutation #1
            ...
            // Save output after first absorption
            let output1 = state;

            // ===== SECOND ABSORPTION: right node =====
            // Absorb right node into rate part, preserve capacity
            for i in 0..RATE {
                state[i] = output1[i] + right[i];  // Add message to rate
            }
            for i in RATE..N_STATE {
                state[i] = output1[i];  // Preserve capacity
            }

            // Poseidon permutation #2
            for round in 0..N_HALF_FULL_ROUNDS {
                for i in 0..N_STATE {
                    state[i] = state[i] + EXTERNAL_ROUND_CONSTS[round][i];
                }
        ...
        let mut leaves: Vec<[BaseField; RATE]> = (0..8)
            .map(|i| std::array::from_fn(|j| BaseField::from_u32_unchecked((i * 10 + j) as u32)))
            .collect();
        ...
        let sibling0_index = if current_index % 2 == 0 { current_index + 1 } else { current_index - 1 };
        ...
        let _parent0 = if current_index % 2 == 0 {
            ...
            hash_two_nodes(level0[sibling0_index], level0[current_index])
        };
        current_index /= 2;
        ...
        let sibling1_index = if current_index % 2 == 0 { current_index + 1 } else { current_index - 1 };
        ...
        let sibling2_index = if current_index % 2 == 0 { current_index + 1 } else { current_index - 1 };
        ...
        println!("  Leaf (first 2 of 8): {:?}", &leaf[0..2]);
        println!("  Leaf (full 8 elements): {:?}", &leaf);
        println!("  Expected root (first 2 of 8): {:?}", &expected_root[0..2]);
        println!("  Expected root (full 8 elements): {:?}", &expected_root);

        // Setup prover
        let config = PcsConfig::default();
        let channel = &mut Blake2sChannel::default();
        let twiddles = SimdBackend::precompute_twiddles(CanonicCoset::new(16).circle_domain().half_coset);
        let commitment_scheme_prover = CommitmentSchemeProver::<SimdBackend, Blake2sMerkleChannel>::new(config, &twiddles);

        // Generate proof
        println!("\n--- Proving ---");
        let (proof, _component_computing, _component_scheduler, statement0, statement1) =
            prove_merkle(depth, leaf, siblings, leaf_index as u32, expected_root, channel, commitment_scheme_prover)
    ...
    fn test_merkle_proof_two_values() {
        ...
        fn hash_two_nodes(left: [BaseField; RATE], right: [BaseField; RATE]) -> [BaseField; RATE] {
            ...
            state[0..RATE].copy_from_slice(&left);
            for round in 0..N_HALF_FULL_ROUNDS {
                for i in 0..N_STATE { state[i] = state[i] + EXTERNAL_ROUND_CONSTS[round][i]; }
                apply_external_round_matrix(&mut state);
                state = std::array::from_fn(|i| pow5(state[i]));
            }
            for round in 0..N_PARTIAL_ROUNDS {
            ...
            for round in 0..N_HALF_FULL_ROUNDS {
                for i in 0..N_STATE { state[i] = state[i] + EXTERNAL_ROUND_CONSTS[round + N_HALF_FULL_ROUNDS][i]; }
                apply_external_round_matrix(&mut state);
                state = std::array::from_fn(|i| pow5(state[i]));
            }
            let output1 = state;
            for i in 0..RATE { state[i] = output1[i] + right[i]; }
            for i in RATE..N_STATE { state[i] = output1[i]; }
            for round in 0..N_HALF_FULL_ROUNDS {
                for i in 0..N_STATE { state[i] = state[i] + EXTERNAL_ROUND_CONSTS[round][i]; }
                apply_external_round_matrix(&mut state);
                state = std::array::from_fn(|i| pow5(state[i]));
        ...
        println!("\nProving leaf contains user_id={}, balance={}", leaf[0], leaf[1]);
        ...
        let (proof, _, _, statement0, statement1) =
            prove_merkle(depth, leaf, siblings, leaf_index as u32, expected_root, channel, commitment_scheme_prover)
                .expect("Proof generation failed");
    ...
    fn test_merkle_detailed_simulation() {
        ...
        println!("L0 = [{}, 0, 0, ...]", l0[0]);
        println!("L1 = [{}, 0, 0, ...]", l1[0]);
        println!("L2 = [{}, 0, 0, ...] ← TARGET (index={})", l2[0], leaf_index);
        println!("L3 = [{}, 0, 0, ...]", l3[0]);
        ...
        println!("    siblings[1] = H01 = [{}, {}, ...]", siblings[1][0], siblings[1][1]);
        ...
        let (proof, _, _, statement0, statement1) =
            prove_merkle(depth, l2, siblings, leaf_index as u32, root, channel, commitment_scheme_prover)
                .expect("Proof generation failed");
