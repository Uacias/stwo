...
use super::{
    PoseidonRelation, N_STATE, RATE, N_HALF_FULL_ROUNDS, N_PARTIAL_ROUNDS, FULL_ROUNDS, N_COLUMNS,
...
pub fn gen_poseidon_computing_interaction_trace(
...

    {
        ...
        for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
            ...
            let final_state_packed: [_; N_STATE] = std::array::from_fn(|i| {
                trace[final_state_col_start + i].data[vec_row]
            });
...
pub fn gen_poseidon_scheduler_trace(
    ...
    let combined_state: [BaseField; N_STATE] = std::array::from_fn(|i| {
        poseidon1_final_state[i] + poseidon2_final_state[i]
    });

    println!("  Scheduler trace:");
    println!("    poseidon1_final_state[0]: {}", poseidon1_final_state[0].0);
    println!("    poseidon2_final_state[0]: {}", poseidon2_final_state[0].0);
...
pub fn gen_poseidon_scheduler_interaction_trace(
...

    {
        ...
        // For each row, use both poseidon1_final_state (columns 0-15) and poseidon2_final_state (columns 16-31)
        ...
        for vec_row in 0..(1 << (log_size - LOG_N_LANES)) {
            ...
            let poseidon2_packed: [_; N_STATE] = std::array::from_fn(|i| trace[N_STATE + i].data[vec_row]);
...
pub fn gen_merkle_computing_interaction_trace(
    ...
    // Column layout (SPONGE): message (8) + initial_state (16) + intermediates (142) + final_state (16)
    // final_state starts at column: 8 + 16 + 142 = 166
    const FINAL_STATE_START: usize = RATE + N_STATE + (N_HALF_FULL_ROUNDS * N_STATE) + N_PARTIAL_ROUNDS + (N_HALF_FULL_ROUNDS * N_STATE);
