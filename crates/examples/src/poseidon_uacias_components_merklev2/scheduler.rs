...
impl FrameworkEval for MerkleSchedulerEval {
    ...
    fn evaluate<E: EvalAtRow>(&self, mut eval: E) -> E {
        ...
        let computed_root_curr: [E::F; RATE] =
            std::array::from_fn(|i| computed_root_curr_vec[i].clone());
        let computed_root_prev: [E::F; RATE] =
            std::array::from_fn(|i| computed_root_prev_vec[i].clone());
        ...
        let expected_root_curr: [E::F; RATE] =
            std::array::from_fn(|i| expected_root_curr_vec[i].clone());
        let expected_root_prev: [E::F; RATE] =
            std::array::from_fn(|i| expected_root_prev_vec[i].clone());

        // Constraint 1: computed_root == expected_root (for all rows, element-wise)
        // This is the main Merkle verification constraint
        for i in 0..RATE {
            eval.add_constraint(computed_root_curr[i].clone() - expected_root_curr[i].clone());
        }
        ...
        for i in 0..RATE {
            eval.add_constraint(
                not_first.clone() * (computed_root_curr[i].clone() - computed_root_prev[i].clone()),
            );
            eval.add_constraint(
                not_first.clone() * (expected_root_curr[i].clone() - expected_root_prev[i].clone()),
            );
        }
