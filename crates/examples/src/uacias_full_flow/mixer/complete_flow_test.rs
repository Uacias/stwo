...
mod tests {
    ...
    use crate::uacias_full_flow::mixer::full_flow::{
        generate_operation, compute_commitment, HybridMerkleTree,
    };
    use crate::uacias_full_flow::{prove_merkle, verify_merkle};
    ...
    fn test_complete_privacy_mixer_flow() {
        ...
        for (i, op) in operations.iter().enumerate() {
            println!("  Op {}: secret={}, nullifier={}, hash={}",
        ...
        let commitments: Vec<_> = operations.iter()
            .map(|op| {
        ...
        let proof_data = tree.get_proof_data(target_commitment)
        ...
        println!("  Verified commitment: {} (index {})", target_commitment.0, proof_data.index);
    ...
    fn test_multiple_verifications() {
        ...
        for (i, &commitment) in commitments.iter().enumerate() {
            ...
            println!("  ✅ Commitment {} verified (index {})", i, proof_data.index);
        }

        println!("\n🎉 All {} commitments verified successfully!", commitments.len());
    }
    ...
    fn test_production_tree_height_32() {
        ...
        println!("✅ Created hybrid tree with height {}", tree_height);
        println!("   Max capacity: 2^{} = {} leaves", tree_height - 1, 1u64 << (tree_height - 1));
        ...
        println!("  Root: {}", proof_data.root.0);
        println!("  Siblings path length: {} (depth = {})", proof_data.siblings.len(), tree_height - 1);
        println!("  First 3 siblings:");
        for (i, sibling) in proof_data.siblings.iter().take(3).enumerate() {
            println!("    Level {}: {} (isRight: {})", i, sibling.0, proof_data.is_right[i]);
        }
        println!("  ...");
        ...
        println!("\n🎉 Successfully verified commitment at index {} in production-scale tree!", proof_data.index);
